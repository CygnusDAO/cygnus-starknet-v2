...
pub mod Collateral {
    ...
    use cygnus::core::collateral::{errors, events};
    use cygnus::core::erc6909::extensions::ERC6909TokenSupplyComponent;
    use cygnus::core::erc6909::{ERC6909Component, ERC6909ABIDispatcher, ERC6909ABIDispatcherTrait};
    ...
    /// We use ERC6909 to represent individual Ekubo positions with the TokenSupply extension to track supplies.
    ...
    fn constructor(
        ...
        let pool_key = PoolKeyCYG { token0, token1, fee, tick_spacing, extension };
    ...
    #[abi(embed_v0)]
    impl Collateral of cygnus::core::collateral::ICollateral<ContractState> {
        /// ---------------------------------------------------------------------------------------------------
        ///                                          TERMINAL
        /// ---------------------------------------------------------------------------------------------------
        ...
        fn deposit(ref self: ContractState, params: DepositParams) -> u128 {
            ...
            // Transfer NFT from the caller into the vault
        ...
        fn redeem(ref self: ContractState, params: RedeemParams) -> (u128, u128) {
            ...
            // Burn the Ekubo position and receive token0/token1
            ...
            self._transfer(self.pool_key().token0, caller, amount0.into());
            self._transfer(self.pool_key().token1, caller, amount1.into());

            //self.underlying.read().transfer_from(get_contract_address(), params.recipient, params.token_id.into());

            /// # Event
            /// * `Withdraw`
            self.emit(events::Withdraw { caller, liquidity: params.liquidity, params });
    ...
    impl TerminalImpl of TerminalTrait {
        fn _transfer(ref self: ContractState, token: ContractAddress, to: ContractAddress, amount: u256) {
            ERC20ABIDispatcher { contract_address: token }.transfer(to, amount);
        }
        ...
        #[inline(always)]
        fn _convert_to_assets(self: @ContractState, id: u64, shares: u128) -> u128 {
            // Gas savings
            let supply = self.erc6909_token_supply.total_supply(id.into());
        ...
        #[inline(always)]
        fn _convert_to_shares(self: @ContractState, id: u64, assets: u128) -> u128 {
            // Gas savings
            let supply = self.erc6909_token_supply.total_supply(id.into());
