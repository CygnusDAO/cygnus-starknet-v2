...
mod Borrowable {
    ...
    use cygnus::core::borrowable::{errors, events};
    ...
    #[abi(embed_v0)]
    impl Borrowable of cygnus::core::borrowable::IBorrowable<ContractState> {
        /// ---------------------------------------------------------------------------------------------------
        ///                                          TERMINAL
        /// ---------------------------------------------------------------------------------------------------
        ...
        fn exchange_rate(self: @ContractState) -> u128 {
            ...
            return self.total_assets().div_wad(supply.try_into().unwrap());
        }
        ...
        fn deposit(ref self: ContractState, assets: u128, recipient: ContractAddress) -> u128 {
            ...
            if (self.erc20.ERC20_total_supply.read() == 0) {
                shares -= 1000;
                self.erc20._mint(Zero::zero(), 1000);
            }

            /// Mint CygUSD
            self.erc20._mint(recipient, shares.into());
        ...
        fn redeem(ref self: ContractState, shares: u128, recipient: ContractAddress, owner: ContractAddress) -> u128 {
            ...
            let caller = get_caller_address();
            if caller != owner {
                self.erc20._spend_allowance(owner, caller, shares.into());
            }
            ...
            self.erc20._burn(owner, shares.into());
        ...
        fn get_lender_position(self: @ContractState, lender: ContractAddress) -> (u128, u128, u128) {
            let cyg_usd_balance: u128 = self.erc20.ERC20_balances.read(lender).try_into().unwrap();
        ...
        fn track_lender(ref self: ContractState, lender: ContractAddress) {
            ...
            let balance = self.balance_of(lender).try_into().unwrap();
        ...
        fn borrow(
            ...
            if borrower != caller {
                self.erc20._spend_allowance(borrower, caller, borrow_amount.into());
            }
    ...
    impl TerminalImpl of TerminalTrait {
        ...
        fn _convert_to_assets(self: @ContractState, shares: u128) -> u128 {
            // Gas savings
            let supply: u128 = self.erc20.ERC20_total_supply.read().try_into().unwrap();
        ...
        fn _convert_to_shares(self: @ContractState, assets: u128) -> u128 {
            // Gas savings
            let supply = self.erc20.ERC20_total_supply.read().try_into().unwrap();
